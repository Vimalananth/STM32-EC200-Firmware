...
//   {pumpFbBase}/alerts/{key}: {event, reason, run_s, relay, ts}  (relay on/off events)
//
...
class _LogsPageState extends State<LogsPage>
  ...
  Future<void> _loadLogs() async {
    ...
    if (!isMaster) {
      final ms = await _db
          .ref('${widget.pumpFbBases.first}/alerts')
  ...
  Future<void> _loadPumpRuntime() async {
    ...
    final masterFbBase = widget.pumpFbBases.first;
    ...
    if (widget.pumpFbBases.length > 1) {
